...
This section is inspired by Chapter 11 of the excellent book "Machine Learning: A Probabilistic Perspective" by Kevin P. Murphy.
...
Fortunately, Mixture of Experts (MoE) (first introduced in {% cite adaptative_mixture_of_experts %}) is a type of model that enables solving this issue. MoEs belong to the class of Latent Variable Models (LVM) that rely on some hidden variables (the latent variables) to make predictions.
...
<div class="caption">
    Illustration of an MDN, the $$ \alpha $$ coefficients corresponding to the estimates of $$ p(z = k \mid x) $$. (Image source: {% cite articulatory_inversion %})
</div>
...
To resolve this issue, {% cite sparse_moe %} introduces two loss terms:
...
<div class="caption">
    Illustration of the implementation of mixture of experts for $$ N_{\text{experts}} = 3 $$ and $$ \text{top}_k = 1 $$ with token dropping. (Image source: {% cite megablocks %})
</div>

We first compute the output of the gating function giving us the probabilities of each expert. We then permute the tokens so as to group them by the expert they are assigned to and keep in memory this permutation (in the case where $$ \text{top}_k > 1 $$, this is not a simple permutation as a token is assigned to multiple experts but can be viewed as a permutation of the tensor where we replicate $$ \text{top}_k $$ times each token).
Finally, we compute the output of each expert and unpermute the tokens (In the case where $$ \text{top}_k > 1 $$, we also compute the average weighted by the gating function outputs for each "duplicated" token).
...
In this case, the matrix $$ M_1 $$ would store the inputs after permutation with a shape $$ (N_{\text{experts}}, \text{capacity}, d) $$ while $$ M_2 $$ would store the expert weights with shape $$ (N_{\text{experts}}, d, d_{\text{ff}}) $$ where $$ d_{\text{ff}} $$ is the width of the middle layer in the FFN (In fact there would be another batched matrix multiply operation with a right operand of shape $$ (N_{\text{experts}}, d_{\text{ff}}, d) $$ as a FFN corresponds to a MLP with 2 layers).
